        ...
        for j in range(size):
            if grid[i][j] == 0:
                ...
                while i + height < size and grid[i + height][j] == 0:
                    width = 1
                    while j + width < size and all(
                        grid[i + k][j : j + width + 1] == [0] * (width + 1)
...
def draw_grid_with_rectangle(size, grid, rectangle_coords):
    root = tk.Tk()
    root.title("Największy prostokąt")
    ...
    if rectangle_coords:
        ...
        for k in range(i + 1, i + height + 1):
            for l in range(j, j + width):
                canvas.create_rectangle(
                    l * 20, k * 20, (l + 1) * 20, (k + 1) * 20, outline="blue", width=2
                )
