...
void log_printf(const char *fmt, ...) {
    ...
    len = snprintf(
        buf,
        sizeof(buf),
        "%02i:%02i:%02i.%06li PID=%ji TID=%ji ",
        date_human.tm_hour,
        date_human.tm_min,
        date_human.tm_sec,
        usec,
...
void log_perror(const char *msg) { log_printf("%s: %s", msg, strerror(errno)); }
...
int accept_verbose(int srv_sock) {
    ...
    if (rv == -1) {
        ...
        log_printf(
            "new client %s:%u, new descriptor %i",
            buf,
            (unsigned int)ntohs(a.sin_port),
            rv
        );
    }
    return rv;
...
ssize_t write_verbose(int fd, void *buf, size_t nbytes) {
    ...
    if (rv == -1) {
        ...
        log_printf(
            "partial write on %i, wrote only %zi of %zu bytes", fd, rv, nbytes
        );
    } else {
